...
def get_pbp(game_id):
    ...
    if not response:
        print(
            "Json pbp for game {} is either not there or can't be obtained".format(
                game_id
            )
        )
        return {}
...
def get_roster(game_json):
    ...
    for player in game_json["roster_player"]:
        players[player["id"]] = " ".join(
            [player["first_name"].strip(), player["last_name"].strip()]
        )
...
def get_goal_players(play, event, players):
    ...
    for ptype in ["plus", "minus"]:
        for num in range(1, 7):
            play["{}Player{}_id".format(ptype_team[ptype], num)] = event[
                "play_actions"
            ][1].get("{}_player_{}".format(ptype, num))

            # Control for None
            if play["{}Player{}_id".format(ptype_team[ptype], num)]:
                play["{}Player{}".format(ptype_team[ptype], num)] = players.get(
                    int(play["{}Player{}_id".format(ptype_team[ptype], num)])
                )
            else:
...
def parse_event(event, score, teams, date, game_id, players):
    ...
    play["seconds_elapsed"] = (
        shared.convert_to_seconds(event["clock_time_string"])
    ...
    play["event"] = (
        event["play_type"]
        if event["play_type"] != "Shootout"
        else event["play_by_play_string"].strip()
    )
    ...
    play["away_goalie"] = players.get(
        int(play["away_goalie_id"]) if play["away_goalie_id"] not in ["", None] else 0
    )
    play["home_goalie"] = players.get(
        int(play["home_goalie_id"]) if play["home_goalie_id"] not in ["", None] else 0
    )
...
def parse_json(game_json, game_id):
    ...
    if len(game_json["plays"]) == 0:
        shared.print_warning(
            "The Json pbp for game {} contains no plays and therefore can't be parsed".format(
                game_id
            )
        )
        return pd.DataFrame()
    ...
    teams = {
        "home": {
            "id": game_json["game"]["home_team"],
            "name": game_json["team_instance"][0]["abbrev"],
        },
        "away": {
            "id": game_json["game"]["away_team"],
            "name": game_json["team_instance"][1]["abbrev"],
        },
    }
    ...
    date = datetime.datetime.strptime(
        date[: date.rfind("-")], "%Y-%m-%dT%H:%M:%S"
    ).strftime("%Y-%m-%d")

    try:
        events = [
            parse_event(play, score, teams, date, game_id, players)
