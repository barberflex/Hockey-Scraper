...
def event_type(play_description: str) -> Optional[str]:
    """
    ...
    for ul in uls:
        ...
        if actual_tm is None:
            shared.print_warning(
                "The team {} in the espn pbp is unknown. We use the supplied team name".format(
                    tm
                )
            )
            actual_tm = tm
...
def parse_espn(espn_xml):
    ...
    try:
        ...
        shared.print_warning(
            "Espn pbp isn't valid xml, therefore coordinates can't be obtained for this game"
        )
    ...
    plays = [
        play for play in plays if play is not None
...
def scrape_game(date, home_team, away_team, game_id=None):
    ...
    try:
        ...
        shared.print_warning(
            "Espn pbp for game {a} {b} {c} is either not there or can't be obtained {d}".format(
                a=date, b=home_team, c=away_team, d=e
            )
        )
        return None
    ...
    try:
        ...
        shared.print_warning(
            "Error parsing Espn pbp for game {a} {b} {c} {d}".format(
                a=date, b=home_team, c=away_team, d=e
            )
        )
        return None
